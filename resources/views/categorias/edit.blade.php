    ...
    <div class="bg-white rounded-xl border border-gray-200 p-6">
      ...
      <form method="POST" action="{{ route('categorias.update', $categoria) }}" class="space-y-6">
          ...
          <div>
            ...
            <select name="id_empresa" class="w-full h-10 rounded-lg border-gray-300 focus:ring-indigo-500 text-sm" required>
              @foreach($empresas as $em)
                <option value="{{ $em->id }}" @selected(old('id_empresa', $categoria->id_empresa)==$em->id)>{{ $em->razon_social }}</option>
              @endforeach
            </select>
        ...
        <div>
          ...
          <textarea name="descripcion" rows="4" class="w-full rounded-lg border-gray-300 focus:ring-indigo-500 text-sm">{{ old('descripcion', $categoria->descripcion) }}</textarea>
        ...
        <div class="flex items-center gap-2">
          <input type="checkbox" name="activa" value="1" id="activa"
                 class="h-4 w-4 text-indigo-600 border-gray-300 rounded" @checked(old('activa', $categoria->activa))>
