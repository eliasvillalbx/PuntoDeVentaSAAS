...
<head>
  ...
  <style>
    ...
    th { background: #fff1f2; font-size: 10px; text-transform: uppercase; color: #9f1239; }
  </style>
</head>
<body>
  @php
  ...
  <div class="grid">
    ...
    <div class="col w-50">
        ...
        <div class="muted">Reporte de Compras</div>
    </div>
    <div class="col w-30 text-right">
        <h2 style="font-size: 16px; margin:0;">Proveedores</h2>
        <div class="muted">{{ date('d/m/Y') }}</div>
    </div>
  </div>
  <table>
    <thead><tr><th>Proveedor</th><th>Contacto</th><th class="text-right">Órdenes</th><th class="text-right">Monto Total</th></tr></thead>
    <tbody>
      @forelse($proveedores as $prov)
        <tr>
          <td><div style="font-weight:bold;">{{ $prov->nombre }}</div><div class="muted">{{ $prov->telefono ?? '' }}</div></td>
          <td>{{ $prov->contacto ?? '--' }}</td>
          <td class="text-right">{{ $prov->cantidad_compras }}</td>
          <td class="text-right"><strong>${{ number_format($prov->total_comprado, 2) }}</strong></td>
        </tr>
      @empty
        <tr><td colspan="4" class="text-center" style="padding:15px;">Sin compras registradas.</td></tr>
      @endforelse
    </tbody>
  </table>
</body>
