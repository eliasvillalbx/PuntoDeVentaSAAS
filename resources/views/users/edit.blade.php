  ...
  <x-slot name="header">
    <div class="flex items-center justify-between">
      <h1 class="text-xl sm:text-2xl font-semibold text-gray-900 flex items-center gap-2">
        <span class="material-symbols-outlined mi text-indigo-600">edit</span>
        Editar Usuario
      </h1>
      ...
      <form method="POST" action="{{ route('users.update', $user) }}" class="p-6 sm:p-8 space-y-10">
        ...
        <section class="space-y-6">
          <div class="border-b border-gray-100 pb-2">
            ...
            <p class="text-xs text-gray-500 mt-1">Actualiza datos y rol permitido.</p>
          </div>
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            ...
            <div>
              ...
              <select name="role" required
                ...
                @foreach($roles as $r)
                  <option value="{{ $r->name }}" @selected(old('role', $currentRole) == $r->name)>{{ str_replace('_',' ', $r->name) }}</option>
                @endforeach
              </select>
