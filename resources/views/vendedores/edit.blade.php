<x-app-layout>
  ...
  <div class="max-w-5xl mx-auto">
    <div class="bg-white rounded-xl border border-gray-200 p-6">
      ...
      <form method="POST" action="{{ route('vendedores.update', $vendedor) }}" class="space-y-8">
        @csrf
        @method('PUT')
        ...
        <section class="space-y-4">
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            <div>
              ...
              <input type="text" name="nombre" value="{{ old('nombre', $vendedor->nombre) }}"
            ...
            <div>
              ...
              <input type="text" name="apellido_paterno" value="{{ old('apellido_paterno', $vendedor->apellido_paterno) }}"
                     class="w-full h-10 rounded-lg border-gray-300 focus:ring-indigo-500 text-sm">
            ...
            <div>
              ...
              <input type="text" name="apellido_materno" value="{{ old('apellido_materno', $vendedor->apellido_materno) }}"
                     class="w-full h-10 rounded-lg border-gray-300 focus:ring-indigo-500 text-sm">
              ...
              <div>
                ...
                <select name="id_empresa" class="w-full h-10 rounded-lg border-gray-300 focus:ring-indigo-500 text-sm">
                  @foreach ($empresas as $em)
                    <option value="{{ $em->id }}" @selected(old('id_empresa', $vendedor->id_empresa) == $em->id)>
                      {{ $em->razon_social }}
                    </option>
                  @endforeach
                </select>
