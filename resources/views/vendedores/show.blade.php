    ...
    <div class="bg-white rounded-xl border border-gray-200 p-6 space-y-6">
      ...
      <section class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        <div>
          ...
          <p class="text-sm text-gray-900 font-medium">
            {{ $vendedor->nombre_completo ?: trim("{$vendedor->nombre} {$vendedor->apellido_paterno} {$vendedor->apellido_materno}") }}
          </p>
        </div>
        <div>
          <h3 class="text-xs text-gray-500 mb-1">Email</h3>
          <p class="text-sm text-gray-900 font-medium">{{ $vendedor->email }}</p>
        </div>
        <div>
          <h3 class="text-xs text-gray-500 mb-1">Teléfono</h3>
          <p class="text-sm text-gray-900 font-medium">{{ $vendedor->telefono ?? '—' }}</p>
        </div>
        <div>
          <h3 class="text-xs text-gray-500 mb-1">Empresa</h3>
          <p class="text-sm text-gray-900 font-medium">{{ $vendedor->empresa?->razon_social ?? '—' }}</p>
        </div>
        <div>
          <h3 class="text-xs text-gray-500 mb-1">Creado</h3>
          <p class="text-sm text-gray-900 font-medium">{{ $vendedor->created_at?->format('Y-m-d H:i') }}</p>
        </div>
        <div>
          <h3 class="text-xs text-gray-500 mb-1">Actualizado</h3>
          <p class="text-sm text-gray-900 font-medium">{{ $vendedor->updated_at?->format('Y-m-d H:i') }}</p>
        </div>
      </section>
      ...
      <section class="pt-4 border-t border-gray-100">
        <form method="POST" action="{{ route('vendedores.destroy', $vendedor) }}"
              onsubmit="return confirm('¿Eliminar este vendedor? Esta acción no se puede deshacer.');">
          @csrf @method('DELETE')
